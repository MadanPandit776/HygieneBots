# Azure NSG IP Limit Bot

## Overview
The Azure NSG IP Limit Bot is a Python script designed to fetch Network Security Groups (NSGs) with source or destination IP counts exceeding a specified threshold across all Azure subscriptions. It utilizes Azure services such as Azure SQL and Azure Identity for authentication and data storage.

## Features

| Feature              | Details                                                 |
|----------------------|---------------------------------------------------------|
| **Naming convention**| azure_nsg_ip_limit                                      |
| **Type**             | Scan-Report                                             |
| **Services**         | Security                                                |
| **Schedule run**     | Weekly                                                  |
...
- Fetches NSGs with source or destination IP counts exceeding a defined threshold.
- Utilizes Azure SQL for storing NSG IP Limit details.
...
## Setup
Follow these steps to set up the Azure NSG IP Limit Bot:
...
## Bot Execution Steps
Follow these guidelines for using the Azure NSG IP Limit Bot:
...
3. **Execution**:
   ...
   - Details on NSGs with source or destination IP counts exceeding the threshold are stored in Azure SQL Database.
...
4. **Monitoring**:
   ...
   - Review Azure SQL Database for NSG IP Limit details.
...
- **Report Email Notifications**: 
  - Additionally, the bot sends a detailed report via email after each execution, summarizing the NSG IP Limit fetched, any exceptions encountered, and the overall execution status. This report provides stakeholders with comprehensive insights into the bot's performance and any potential issues that need attention.

## Additional Documentation
For detailed instructions on configuring NSG IP Limit, refer to the [Microsoft Azure documentation]https://learn.microsoft.com/en-us/answers/questions/1350706/maximum-number-of-ips-in-a-nsg-rule).
