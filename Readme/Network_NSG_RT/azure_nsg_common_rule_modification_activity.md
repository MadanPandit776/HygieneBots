# Azure NSG Common Rule Modification Bot

## Overview
The Azure NSG Common Rule Modification Bot is a Python script designed to track changes to Network Security Groups (NSGs) within a specified timeframe across all Azure subscriptions. It helps in identifying modifications made to NSG rules, including creation, updates, and deletions, and the rule's priority is greater than 3500. It utilizes Azure services such as Azure SQL and Azure Identity for authentication and data storage.

## Features

| Feature              | Details                                                 |
|----------------------|---------------------------------------------------------|
| **Naming convention**| azure_nsg_common_rule_modification_activity             |
| **Type**             | Scan-Report                                             |
| **Services**         | Security                                                        |
| **Schedule run**     | Daily                                                        |
...
- Fetches modifications to NSG Common rules, including creation, update, and deletion events.
- Utilizes Azure SQL for storing NSG Common rule modification details.
...
## Setup
Follow these steps to set up the Azure NSG Common Rule Modification Bot:
...
## Bot Execution Steps
Follow these guidelines for using the Azure NSG Common Rule Modification Bot:
...
3. **Execution**:
   ...
   - Details on NSG Common rule modification are stored in Azure SQL Database.
...
4. **Monitoring**:
   ...
   - Review Azure SQL Database for NSG Common rule modification details.

   ## Email Notification (Regular/Failure)
The Azure NSG Common Rule Modification Bot includes email notification functionality to keep stakeholders informed about the execution status and any potential issues. Here's how it works:
...
- **Report Email Notifications**: 
  - Additionally, the bot sends a detailed report via email after each execution, summarizing the NSG Common rule modification fetched, any exceptions encountered, and the overall execution status. This report provides stakeholders with comprehensive insights into the bot's performance and any potential issues that need attention.

## Additional Documentation
For detailed instructions on configuring NSG Common rule modification, refer to the [Microsoft Azure documentation](https://learn.microsoft.com/en-us/azure/virtual-network/network-security-groups-overview).
