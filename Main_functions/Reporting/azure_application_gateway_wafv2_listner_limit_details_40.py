...
def check_appgw_listener_waf_v2_limit():
    try:
            ...
            try:
                ...
                for appgw in app_gateways:
                    ...
                    if appgw.web_application_firewall_configuration:
                        ...
                        if waf_config.enabled:  # Check if WAF is enabled
        ...
        table_name = 'azure_application_gateway_wafv2_listner_limit_details_40_details'
        container_name = 'azure-application-gateway-wafv2-listner-limit-40-details'
        ...
        if all_logs:
            ...
            Azure_Blob_Convertion.Blob_function(all_logs_df, 'azure-application-gateway-wafv2-listner-limit-40-details', 'all_logs')
            logger.info(f"All logs generated for CSV")

        if error_logs:
            error_logs_df = pd.DataFrame(error_logs)
            Azure_Blob_Convertion.Blob_function(error_logs_df, 'azure-application-gateway-wafv2-listner-limit-40-details', 'error_logs')
            logger.info(f"Error logs generated for CSV")
            notifications_email.send_email('Exception log file generated', 'azure-application-gateway-wafv2-listner-limit-40-details' +' Exception Report', "excel", 'azure-application-gateway-wafv2-listner-limit-40-details', error_logs_df)
